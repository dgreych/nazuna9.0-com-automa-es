...
try {
  ...
  const result = await makeAssistentRequest(
    {
      mensagens: [
        {
          texto: 'Responda apenas com a frase: FLUXO NAZUNA OK',
          id_enviou: testUser,
  ...
  if (texts.length === 0) {
    throw new Error(`A resposta não contém texto utilizável: ${JSON.stringify(result)}`);
  }
  ...
  // O banco usa gravação com debounce de 2 segundos.
